...
public class AgentServiceImpl implements AgentService {
    ...
    public AgentResponseDto getAgentById(long agentId) {
        Agent agent = agentRepository.findByIdOptional(agentId).orElseThrow(() -> new AgentNotFoundException(String.format("Agent with id %d not found", agentId)));
    ...
    public Agent findByUsername(String username) {
        return agentRepository.findByUsername(username).orElseThrow(() -> new AgentNotFoundException(String.format("Agent with username %s not found", username)));
    }
